...
import { siteWidgetFormSchema, type SiteWidgetFormValues, type SiteWidgetConfig } from '../../types'
  ...
  return (
    <>
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? '编辑卡片小工具' : '新增卡片小工具'}</DialogTitle>
          <DialogDescription>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <SelectField
              ...
              options={[
                ...
                { label: '仅首页侧边栏 (Home Sidebar)', value: 'home_sidebar' },
                { label: '仅内容单页侧边栏 (Page Sidebar)', value: 'page_sidebar' },
                { label: '全前台侧边栏 (首页与单页均展示)', value: 'site_sidebar' },
                { label: '仅后台仪表盘 (Admin Dashboard)', value: 'dashboard' },
              ]}
              required
            />
            <SelectField
              ...
              options={[
                ...
                { label: '图片超链接海报 (Image Banner)', value: 'image_banner' },
          ...
          {cardType === 'image_banner' && (
            <div className="space-y-3 p-3 bg-muted/30 rounded-lg border">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-foreground">横幅海报图片</span>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs text-primary px-2 gap-1 border-primary/30 hover:bg-primary/10"
